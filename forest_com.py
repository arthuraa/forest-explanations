...
X = original_data[original_data.columns.difference(communities.uninformative)]
X = X[X.columns.difference(communities.race_features)]
del X['ViolentCrimesPerPop']
X = X.fillna(-1)
...
class ForestClusters():
    ...
    def summarize(self, X, y, path="."):
        leaves = self.model.apply(X)
        activations = self.encoder.transform(leaves)
        clusters = self.kmeans.predict(activations)
        distances = self.kmeans.transform(activations)
